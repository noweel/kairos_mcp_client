...
Codex·Antigravity처럼 플러그인 체계가 없는 클라이언트는 `kairos/scripts/kairos-client`를 복사해 `replay --transcript`로 명시 보관하고, MCP 등록은 각자의 설정에서 합니다. 대화록 어댑터는 Claude Code 것만 있습니다. 서버 쪽 설치와 운용 전체는 `deploy/README.md`에 있습니다.
