...
> 이 저장소(`noweel/kairos_mcp_client`)는 KAIROS의 **클라이언트·배포 묶음**이다: Claude Code 플러그인, Codex CLI 설치기, systemd 유닛, 그리고 이 문서. 코어(`noweel/kairos`)의 작업 트리에서는 `deploy/`로 체크아웃한다(`decisions.md` §128).
...
cp systemd/kairos.env.example ~/.config/kairos/kairos.env
...
뷰어는 `http://<서버>:8080`이다. 토큰은 한 번 넣으면 브라우저에 남으므로(`localStorage`, decisions.md §79) 재접속 때 다시 넣지 않는다. 폰에서는 `http://<서버>:8080/#token=<토큰>`을 열면 손으로 옮기지 않아도 되고, 값은 열자마자 주소에서 지워진다. 지우는 길은 화면의 로그아웃이다. 비밀번호 방식은 검토했으나 채택하지 않았다: 불편의 원인은 토큰이 아니라 보관 수명이었다. Windows·모바일에서는 이 주소를 브라우저로 열면 되고, **KAIROS를 그쪽에 설치하지 않는다.**
...
- **`notes/`·`meta/`는 git이 맡는다** — 수신기의 배경 작업 `backup()`이 `last_accessed`를 파일에 먼저 반영한 뒤 하루 한 번 커밋하고 `~/kairos-data/vault.git`에 밀어 넣는다(Store §8). 하루에 한 번인지는 마지막 커밋 날짜로 git이 판단한다.
